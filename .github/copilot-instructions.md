...
- **Build system**: Uses `build-adapter ts` (via @iobroker/adapter-dev) for TypeScript compilation
...
tests.integration(path.join(__dirname, '..'), {
    defineAdditionalTests({ suite }) {
        suite('Test adapter with specific configuration', (getHarness) => {
            ...
            it('should configure and start adapter', async function () {
                // Get adapter object
                const obj = await new Promise((res, rej) => {
                    harness.objects.getObject('system.adapter.ico-cloud.0', (err, o) => {
                ...
                // Verify states
                const stateIds = await harness.dbConnection.getStateIDs('ico-cloud.0.*');

                if (stateIds.length > 0) {
                    console.log('✅ Adapter successfully created states');
                    await harness.stopAdapter();
                } else {
                    throw new Error('Adapter did not create any states');
                }
            }).timeout(40000);
        });
    }
...
```javascript
it('should fail gracefully with invalid OAuth2 tokens', async function () {
    harness = getHarness();
    const obj = await new Promise((res, rej) => {
        harness.objects.getObject('system.adapter.ico-cloud.0', (err, o) => {
            if (err) return rej(err);
    ...
    // Configure with invalid tokens to test error handling
    ...
    const state = await harness.states.getStateAsync('ico-cloud.0.info.connection');

    if (!state || state.val === false) {
        console.log('✅ Adapter correctly failed with invalid tokens');
    } else {
        throw new Error('Expected adapter to fail but it succeeded');
    }
...
#### ICO Cloud OAuth2 Testing Pattern

- The ICO Cloud adapter uses OAuth2 tokens (`accessToken`/`refreshToken`) instead of username/password
- Tokens are encrypted via `encryptedNative` and `protectedNative` in io-package.json
- Create separate test file: `test/integration-demo.js`
- Add npm script: `"test:integration-demo": "mocha test/integration-demo --exit"`

**Example Implementation:**
```javascript
it("Should connect to Ondilo API with valid OAuth2 tokens", async () => {
    // Note: OAuth2 tokens must be pre-obtained via Ondilo's OAuth2 flow
    await harness.startAdapter();
    await new Promise(resolve => setTimeout(resolve, 60000));

    const connectionState = await harness.states.getStateAsync("ico-cloud.0.info.connection");

    if (connectionState?.val === true) {
        console.log("✅ SUCCESS: Ondilo API connection established");
        return true;
    } else {
        throw new Error("API Test Failed: Expected Ondilo API connection. Check logs for OAuth2 errors.");
    }
}).timeout(120000);
```
...
- **Avoid:** `axios` unless specific features are required. Note: this adapter currently uses `axios` for OAuth2 token handling — future refactoring to native `fetch` is encouraged but not required
...
**ICO Cloud OAuth2 Configuration Example:**
```json
{
  "type": "panel",
  "items": {
...
* (garfonso) **FIXED**: Adapter now properly handles OAuth2 token refresh (fixes #25)
* (garfonso) **NEW**: Added support for additional pool measurement types
```
...
- **Build the adapter**: `npm run build` (uses `build-adapter ts` via @iobroker/adapter-dev)
- **Type checking**: `npm run check` -- runs TypeScript compiler without emitting
- **Watch mode**: `npm run watch` -- continuously rebuilds on changes
- **Lint code**: `npm run lint` -- runs ESLint with @iobroker/eslint-config
- **Run all tests**: `npm test` -- runs TypeScript unit tests and package tests
- **Release**: `npm run release` -- uses @alcalzone/release-script

### ICO Cloud API Architecture
- All Ondilo API interactions are in `src/lib/api.ts`
- Uses OAuth2 authentication with encrypted token storage (`refreshToken`, `accessToken`)
- Tokens are encrypted via `encryptedNative` and `protectedNative` in io-package.json
- API calls are rate-limited (5 requests/second, 30 requests/hour)
- Measurement types: temperature, ph, orp, salt, tds, battery, rssi
- Pool recommendations are stored as ioBroker states

### OAuth2 Flow
- The adapter uses Ondilo's OAuth2 flow via admin UI (admin/jsonConfig.json)
- OAuth2 endpoints: `https://interop.ondilo.com/oauth2/authorize` and `https://interop.ondilo.com/oauth2/token`
- Tokens are stored encrypted in the adapter's native configuration
- Token refresh is handled automatically in `src/lib/api.ts`

### Scheduling
- Adapter runs as a schedule adapter (mode: "schedule")
- Default schedule: `59 * * * *` (once per hour at 59 minutes)
- `allowInit: true` enables running on adapter start